...
public:
    ...
    void clusterHelper(int i, std::vector<int>& cluster, typename pcl::PointCloud<PointT>::Ptr cloud, std::vector<bool>& processed ,KdTree* tree, float distanceTol);
    std::vector<std::vector<int>> euclideanCluster(const typename pcl::PointCloud<PointT>::Ptr& cloud, KdTree* tree, float distanceTol);
    std::vector<typename pcl::PointCloud<PointT>::Ptr> KdClustering(typename pcl::PointCloud<PointT>::Ptr cloud, float disTol, int min, int max);
